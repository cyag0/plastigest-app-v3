...
export default function ClientsForm(props: ClientsFormProps) {
  const params = useLocalSearchParams();
  const clientId =
    props.id || (params.id ? parseInt(params.id as string) : undefined);
  const isEditing = !!clientId;
  const formRef = useRef<AppFormRef<CustomerFormData>>(null);
    ...
    >
      <ScrollView style={{ flex: 1 }}>
        <View style={{ padding: 16 }}>
          ...
          <FormCheckBox name="is_active" text="Cliente Activo" />
        </View>
      </ScrollView>
    </AppForm>
  );
